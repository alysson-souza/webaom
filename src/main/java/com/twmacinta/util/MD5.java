 ...
 *
 * <p>Originally written by Santeri Paavolainen, Helsinki Finland 1996.<br>
 ...
 * USA.
 *
 * <p>See http://www.twmacinta.com/myjava/fast_md5.php for more information on this file and the
 * related files.
 *
 * <p>This was originally a rather straight re-implementation of the reference implementation given
 * in RFC1321 by RSA. It passes the MD5 test suite as defined in RFC1321.
 *
 * <p>Many optimizations made by Timothy W Macinta. Reduced time to checksum a test file in Java
 * alone to roughly half the time taken compared with java.security.MessageDigest (within an
 * intepretter). Also added an optional native method to reduce the time even further. See
 * http://www.twmacinta.com/myjava/fast_md5.php for further information on the time improvements
 * achieved.
 *
 * <p>Some bug fixes also made by Timothy W Macinta.
 *
 * <p>Please note: I (Timothy Macinta) have put this code in the com.twmacinta.util package only
 * because it came without a package. I was not the the orig author of the code, although I did
 * optimize it (substantially) and fix some bugs.
 *
 * <p>This Java class has been derived from the RSA Data Security, Inc. MD5 Message-Digest Algorithm
 * and its reference implementation.
 *
 * <p>This class will attempt to use a native method to quickly compute checksums when the
 * appropriate native library is available. On Linux, this library should be named "MD5.so" and on
 * Windows it should be named "MD5.dll". The code will attempt to locate the library in the
 * following locations in the order given:
 *
 * <ol>
 *   <li>The path specified by the system property "com.twmacinta.util.MD5.NATIVE_LIB_FILE" (be sure
 *       to include "MD5.so" or "MD5.dll" as appropriate at the end of the path).
 *   <li>A platform specific directory beneath the "lib/arch/" directory. On Linux for x86, this is
 *       "lib/arch/linux_x86/". On Windows for x86, this is "lib/arch/win32_x86/".
 *   <li>Within the "lib/" directory.
 *   <li>Within the current directory.
 * </ol>
 *
 * <p>If the library is not found, the code will fall back to the default (slower) Java code.
 *
 * <p>As a side effect of having the code search for the native library, SecurityExceptions might be
 * thrown on JVMs that have a restrictive SecurityManager. The initialization code attempts to
 * silently discard these exceptions and continue, but many SecurityManagers will attempt to notify
 * the user directly of all SecurityExceptions thrown. Consequently, the code has provisions for
 * skipping the search for the native library. Any of these provisions may be used to skip the
 * search as long as they are performed <i>before</i> the first instance of a com.twmacinta.util.MD5
 * object is constructed (note that the convenience stream objects will implicitly create an MD5
 * object).
 *
 * <p>The first option is to set the system property "com.twmacinta.util.MD5.NO_NATIVE_LIB" to
 * "true" or "1". Unfortunately, SecurityManagers may also choose to disallow system property
 * setting, so this won't be of use in all cases.
 *
 * <p>The second option is to call com.twmacinta.util.MD5.initNativeLibrary(false) before any MD5
...
public class MD5 extends jonelo.jacksum.algorithm.AbstractChecksum {
    ...
    private void Decode(byte buffer[], int shift, int[] out) {
        ...
        // unrolled loop (orig loop shown above)
     ...
     *
     * @param stt Which state is updated
    ...
    public void Update(MD5State stat, byte buffer[], int offset, int len) {
        ...
        if ((len - offset) > buffer.length) len = buffer.length - offset;
        ...
        if (len >= partlen) {
             ...
             */ {
                ...
                if (partlen == 64) partlen = 0;
                else {
                    for (i = 0; i < partlen; i++) stat.buffer[i + index] = buffer[i + offset];
        ...
        if (i < len) {
            start = i;
            for (; i < len; i++) stat.buffer[index + i - start] = buffer[i + offset];
        }
    }
     ...
     * System.out.println(native_lib_loaded);
     * native_lib_init_pending = false;
     * }
     *
