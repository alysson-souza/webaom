...
public class Cache {
    ...
    public Cache() {
        m_hm = new MyMap[3];
        for (int i = 0; i < m_hm.length; i++) m_hm[i] = new MyMap();
    }
    ...
    public void clear() {
        for (int i = 0; i < m_hm.length; i++) m_hm[i].clear();
    }
    ...
    public synchronized String gatherInfo(Job j, boolean tree) { // for new files
        ...
        try {
            ...
            if (f.gid != 0) f.group = (Group) get(f.gid, DB.I_G);
    ...
    public void rebuildTree() {
        ...
        A.p.clear();
        Job[] j = A.jobs.array();
        for (int i = 0; i < j.length; i++) treeAdd(j[i]);
    ...
    public void treeAdd(Job j) {
        ...
        if (j.incompl() || (hideE && !m) || (hideN && m) || (j.hide(A.preg))) return;
        AFile f = j.m_fa;
        Anime a = f.anime;
        if (A.p.has(a)) A.p.remove(a);
        switch (mImode) {
            case I_MAF:
                {
                    a.add(f);
