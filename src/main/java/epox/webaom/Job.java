...
public class Job {
    ...
    public boolean hide(String s) {
        ...
        if (s.charAt(0) == '!') return m_fc.getAbsolutePath().matches(s.substring(1));
    ...
    public String getStatusText() {
        if (check(H_NORMAL)) return statusStr(getStatus());
    ...
    public void setStatus(int status, boolean test) {
        ...
        if ((status & F_DB) == F_DB) { // only for main status
            ...
            health = H_NORMAL;
            if (status == FINISHED && !m_fc.exists()) health = H_MISSING;
    ...
    private void setHealth(int health) {
        int s = getStatus();
        if (!check(H_NORMAL) && health == H_NORMAL) A.jobs.updateQueues(this, 0, s);
        else if (check(H_NORMAL) && health != H_NORMAL) A.jobs.updateQueues(this, s, -1);
    ...
    public void updateHealth(int i) {
        if (i != H_DELETED && checkOr(H_MISSING | H_DELETED)) return;
        int health = getHealth();
        switch (i) {
            case H_PAUSED:
                if (health == H_NORMAL) health = H_PAUSED; // turn pause on
                else if (health == H_PAUSED) health = H_NORMAL; // turn pause off
                break;
            case H_DELETED:
                if (health == H_DELETED) {
