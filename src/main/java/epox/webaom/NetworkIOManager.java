	...
	private void identify(Job job) throws AniDBException {
		...
		if (job.anidbFile == null) {
			...
			if (job.fileIdOverride > 0) {
				fileData = AppContext.conn.retrieveFileData(job.fileIdOverride, job.getFile().getName());
			} else {
				fileData = AppContext.conn.retrieveFileData(job.fileSize, job.ed2kHash, job.getFile().getName());
			}
			...
			if (job.anidbFile.episode == null) {
				job.anidbFile.episode = (Episode) AppContext.cache.get(job.anidbFile.episodeId,
						DatabaseManager.INDEX_EPISODE);
			}
			if (job.anidbFile.group == null) {
