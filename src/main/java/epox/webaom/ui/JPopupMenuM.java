...
public class JPopupMenuM extends JPopupMenu implements MouseListener, ActionListener {
    ...
    public JPopupMenuM(final JTable jt, final RowModel jlm) {
        ...
        for (int i = 0; i < iCommands; i++)
            if (separator(i)) this.addSeparator();
            else {
    ...
    public void mousePressed(MouseEvent e) {
        if (jt instanceof JTableJobs) ((JTableJobs) jt).upd = false;
    }

    public void mouseReleased(MouseEvent e) {
        if (jt instanceof JTableJobs) ((JTableJobs) jt).upd = true;
    }
    ...
    void command(int cmd, Job[] rows, String arg) {
        ...
        if (cmd == EDIT_PATH) {
            arg =
                    new JTextInputDialog(A.frame, "Edit path", rows[0].getFile().getParent())
                            .getStr();
            if (arg == null || arg.length() < 2) return;
        }
        for (int i = 0; i < rows.length; i++) if (rows[i] != null) command0(cmd, rows[i], arg);
    }
    ...
    void command0(int cmd, Job j, String arg) {
        switch (cmd) {
            ...
            case SET_FINISHED:
                ...
                break;
            case SET_FOLDER:
                if (arg != null) JobMan.setPath(j, arg, false);
                break;
            case SET_PAR_FLD:
                if (arg != null) JobMan.setPath(j, arg, true);
                break;
            case REMOVE_DB:
    ...
    void commandSingle(int cmd, Job j) {
        switch (cmd) {
            ...
            case EDIT_NAME:
                JobMan.setName(
                        j,
                        new JTextInputDialog(A.frame, "Edit name", j.getFile().getName()).getStr());
                break;
            case SET_FID:
