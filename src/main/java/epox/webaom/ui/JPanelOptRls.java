...
public class JPanelOptRls extends JPanel implements Action, ActionListener, ItemListener {
    ...
    public JPanelOptRls(Rules rules) {
        ...
        try {
            ...
            saxParser.parse(
                    WebAOM.class.getClassLoader().getResourceAsStream("rule-helper.xml"), handler);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (obj == jtDef) removeElements(rmDef.getData(), jtDef.getSelectedRows());
        else if (obj == jbOK) testAndApplyRules();
        jtDef.updateUI();
    }
    ...
    public void itemStateChanged(ItemEvent e) {
        ...
        if (e.getStateChange() == ItemEvent.DESELECTED) {
            if (obj == jrbMov) rules.setMov(jta.getText());
            else // if(obj == jrbRen)
            rules.setRen(jta.getText());
        } else if (obj == jrbMov) jta.setText(rules.getMov());
        else // if(obj == jrbRen)
    ...
    public void testAndApplyRules() {
        ...
        if (jrbRen.isSelected()) rules.setRen(text);
        else rules.setMov(text);
    }

    public void updateRules() {
        if (jrbRen.isSelected()) jta.setText(rules.getRen());
