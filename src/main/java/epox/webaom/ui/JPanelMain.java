...
public class JPanelMain extends JPanel
    ...
    private void init() {
        ...
        JPanelCommand jcp =
                new JPanelCommand(
                        cm,
                        "Implemented commands: !uptime,!mystats,!anime,!group,!randomanime,!mylist,!state,!watched,!storage,!font\nTo test API directly start command with '?'.\nLogin/logout is done automatically, no need to set s=.\n");
        cm.setLog(jcp);
    ...
    public void fatal(boolean b) {
        if (b) {
            ...
            for (int i = 1; i < jbButt.length; i++) jbButt[i].setEnabled(false);
            mTunf.start();
        } else {
            mTunf.stop();
            setEnabled_dio(true);
            setEnabled_nio(true);
            for (int i = 1; i < jbButt.length; i++) jbButt[i].setEnabled(true);
        }
    }
    ...
    public void hyperlinkUpdate(HyperlinkEvent e) {
        if (e.getEventType() == HyperlinkEvent.EventType.ACTIVATED) hlGo(e.getDescription());
        else if (e.getEventType() == HyperlinkEvent.EventType.ENTERED) {
            mSmsg = jpb0.getString();
            jpb0.setString(e.getDescription());
        } else if (e.getEventType() == HyperlinkEvent.EventType.EXITED) jpb0.setString(mSmsg);
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (source == jbButt[B_SELF]) selectFiles();
            ...
            jtfNewExt.setText("");
        } else if (source == jpOdiv.tfLogfl) startLog();
    ...
    private void startLog() {
        if (jepM.openLogFile(jpOdiv.tfLogfl.getText())) jpOdiv.tfLogfl.setEnabled(false);
    }
    ...
    private boolean nioEnableInt() {
        if (!killed() && mWnio == null) {
            ...
            if (new JDialogLogin().getPass() == null) return false;
    ...
    public void println(Object o) {
        ...
        if (jepM.isVisible()) javax.swing.SwingUtilities.invokeLater(mRjsd);
    }
    ...
    public void updateProgressbar() {
        ...
        if (A.frame != null) A.frame.setTitle("WebAOM " + A.S_VER + " " + A.jobc.getStatus());
        if (((mIupds++) % 10) == 0) System.gc();
    }
    ...
    public void hlGo(String url) {
        ...
        try {
            ...
            if (path.length() > 0) rt.exec(new String[] {path, url});
            else rt.exec(new String[] {"rundll32", "url.dll,FileProtocolHandler", url});
    ...
    private void selectFiles() {
        ...
        if (option == JFileChooser.APPROVE_OPTION) select(fc.getSelectedFiles());
    ...
    private class RecDir extends Thread {
        ...
        public void run() {
            ...
            for (int i = 0; i < dirs.length; i++) {
                ...
                else nr += addFileRecursive(dirs[i]);
            }
            if (print) {
                String s = "";
                if (nr == 1) s = "Added one file in " + (System.currentTimeMillis() - t0) + " ms.";
                else if (nr > 1)
        ...
        private int addFileRecursive(File file) {
            ...
            if (file.isDirectory()) {
                ...
                for (int i = 0; i < files.length; i++) nr += addFileRecursive(files[i]);
                return nr;
            }
        ...
        public void run() {
            ...
            if (A.db._init(jt.getText())) {
                ...
                for (int i = 0; i < o.length; i++) {
                    ...
                    if (j.getStatus() == Job.MOVEWAIT) JobMan.updatePath(j);
                    // updateJobTable(j);
                }
