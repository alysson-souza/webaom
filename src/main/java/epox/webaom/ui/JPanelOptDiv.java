...
public class JPanelOptDiv extends JPanel {
    ...
    public JPanelOptDiv() {
        ...
        tfMyDBs.setToolTipText(
                "JDBC url (e.g. jdbc:postgresql://localhost:5432/webaom?user=root&password=pass). If empty, H2 embedded database will be used. Press enter to connect.");
    ...
    public void setEnabled(boolean b) {
        ...
        for (int i = 1; i < cbHsh.length; i++) cbHsh[i].setEnabled(b);
    }
    ...
    public HashContainer getHashContainer() {
        try {
            int nr = 0;
            for (int i = 0; i < cbHsh.length; i++) if (cbHsh[i].isSelected()) nr++;
            ...
            if (cbHsh[H_MD5].isSelected()) hc.add(nr++, "md5", new com.twmacinta.util.MD5());
            if (cbHsh[H_SHA].isSelected())
                hc.add(nr++, "sha1", new jonelo.jacksum.algorithm.MD("SHA-1"));
            if (cbHsh[H_TTH].isSelected()) hc.add(nr++, "tth", new TTH());
            return hc;
