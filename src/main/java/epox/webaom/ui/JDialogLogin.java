...
public class JDialogLogin extends JDialog implements ActionListener {
    ...
    private void init() {
        ...
        ok = new JButton("OK");
        ok.setToolTipText(
                "<html>Login is required to access the <i>AniDB UDP Service</i> which enables<br>WebAOM to identify files and add them to your MyList.</html>");
        ...
        if (usr.getText().length() > 0) psw.requestFocus();
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        keyt = key.getText();
        if (usrt.length() < 3) ok.setText("Username too short - OK");
        else if (usrt.length() > 16) ok.setText("Username too long - OK");
        else if (!U.alfanum(usrt)) ok.setText("Only letters and digits - OK");
        else if (pswt.length() < 4) ok.setText("Password too short - OK");
    ...
    public UserPass getPass() {
        setVisible(true);
        if (success) return new UserPass(usrt, pswt, keyt);
        return null;
    }
}
