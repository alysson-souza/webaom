...
public class JobList {
    ...
    public JobList() {
        ...
        for (int i = 0; i < m_lh.length; i++) m_lh[i] = new LinkedHash();
    }
    ...
    public synchronized void clear() {
        ...
        m_al.clear();
        m_hs.clear();
        for (int i = 0; i < m_lh.length; i++) m_lh[i].clear();
        // mIid = 0;
    }
    ...
    public synchronized void filter(int status, int state, boolean unk) {
        ...
        for (int i = 0; i < m_al.size(); i++) {
            j = (Job) m_al.get(i);
            if (j.checkSep(status, state, unk)) al.add(j);
            // j.mIid = i;
        }
    ...
    private void updateHashSets(Job j, int status, boolean add) {
        ...
        if (A.bitcmp(status, Job.S_DO) || A.bitcmp(status, Job.S_DOING)) {
            if (A.bitcmp(status, Job.D_DIO)) type = DIO;
            else if (A.bitcmp(status, Job.D_NIO)) type = NIO;
            else return;
