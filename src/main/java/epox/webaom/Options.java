...
public class Options {
    ...
    public boolean equals(Options o) {
        for (int i = 0; i < B_LEN; i++) if (mBa[i] != o.mBa[i]) return false;
        for (int i = 0; i < I_LEN; i++) if (mIa[i] != o.mIa[i]) return false;
        for (int i = 0; i < S_LEN; i++) if (!strcmp(mSa[i], o.mSa[i])) return false;
        return true;
    }
    ...
    private String encB() {
        String bool = "";
        for (int i = 0; i < B_LEN; i++) bool += mBa[i] ? '1' : '0';
        return bool;
    }

    private void decB(String bool) {
        char[] c = bool.toCharArray();
        for (int i = 0; i < B_LEN; i++) mBa[i] = (c[i] == '1');
    }

    private String encI() {
        String ints = "";
        for (int i = 0; i < I_LEN; i++) ints += mIa[i] + Options.S_SEP;
        return ints;
    }

    private void decI(String str) {
        StringTokenizer st = new StringTokenizer(str, Options.S_SEP);
        for (int i = 0; i < I_LEN; i++) mIa[i] = Integer.parseInt(st.nextToken());
    }
