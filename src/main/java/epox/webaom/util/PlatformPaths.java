...
public class PlatformPaths {
     ...
     *
     * <p>macOS: ~/Library/Logs/webaom/ Linux: ~/.local/share/webaom/logs/ Windows:
    ...
    public static String getDefaultDatabaseDirectory() {
        if (IS_MAC) {
            return new File(USER_HOME, "Library/Application Support/" + APP_NAME + "/database")
                    .getAbsolutePath();
        } else if (IS_WINDOWS) {
            String appData = System.getenv("APPDATA");
            if (appData != null && !appData.isEmpty()) {
            ...
            if (xdgDataHome != null && !xdgDataHome.isEmpty()) {
                ...
                return new File(USER_HOME, ".local/share/" + APP_NAME + "/database")
                        .getAbsolutePath();
            }
        }
        // Fallback for unknown platforms
