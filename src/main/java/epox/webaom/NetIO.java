...
public class NetIO implements Runnable {
    ...
    private void do_work() throws AConEx, InterruptedException {
        ...
        if (A.conn.login()) {
            ...
            do {
                ...
                if (m_job != null) {
                    // !A.nr_nio = m_job.mIid;
                    if (m_job.getStatus() == Job.REMWAIT) remove(m_job);
                    else {
                        if (m_job.getStatus() == Job.IDENTWAIT) identify(m_job);
                        if (A.gui.nioOK() && m_job.getStatus() == Job.ADDWAIT) mylistAdd(m_job);
                    }
    ...
    private void identify(Job j) throws AConEx {
        ...
        if (j.m_fa == null) {
            String[] s = null;
            if (j.mIfid > 0) s = A.conn.retrieveFileData(j.mIfid, j.getFile().getName());
            else s = A.conn.retrieveFileData(j.mLs, j._ed2, j.getFile().getName());
            if (s != null && A.cache.parseFile(s, j) != null) {
                j.mIlid = j.m_fa.lid;
                j.m_fa.setJob(j);
                A.gui.println(
                        "Found "
                                + Hyper.name(j.m_fa.def)
                                + " "
            ...
            } else JobMan.updateStatus(j, Job.UNKNOWN);
        } else {
            if (j.m_fa.group == null) j.m_fa.group = (Group) A.cache.get(j.m_fa.gid, DB.I_G);
            if (j.m_fa.ep == null) j.m_fa.ep = (Ep) A.cache.get(j.m_fa.eid, DB.I_E);
            if (j.m_fa.group == null) j.m_fa.anime = (Anime) A.cache.get(j.m_fa.aid, DB.I_A);
            JobMan.updateStatus(j, Job.IDENTIFIED);
        }
    }
    ...
    public boolean ping(ACon ac) {
        try {
            A.gui.println(
                    "AniDB is reachable. Received reply in "
                            + Hyper.number("" + ac.enCrypt())
                            + " ms.");
            return true;
