...
public class ChiiEmu implements CommandModel {
    ...
    public void handleCommand(String cmd) {
        ...
        } else if (m_ac == null) println("No connection");
        else if (mTw == null) {
    ...
    private class EmuWorker extends Thread {
        ...
        public String stosta(String p, int t) throws AConEx {
            ...
            for (int i = 0; i < a.length; i++) {
                a[i] = a[i].trim();
                if (i < (a.length - 1) && a[i].length() < 1) return getS(t);
            }
            ...
            try {
                sb.append("&epno=");
                if (a[2].equalsIgnoreCase("all")) sb.append("0");
            ...
            if (I_STORA == t) {
                ...
                cmd = "STATE2";
                sb.append("&state=");
                if (a[3].equalsIgnoreCase("unknown")) sb.append(0);
                else if (a[3].equalsIgnoreCase("hdd")) sb.append(1);
                else if (a[3].equalsIgnoreCase("cd")) sb.append(2);
                else if (a[3].equalsIgnoreCase("deleted")) sb.append(3);
            ...
            if (r.data.equals("1")) return cmd + ": one entry updated";
            return cmd + ": updated " + r.data + " entries";
        }

        public String randomanime(String param) throws AConEx {
            int type = 0;
            if (param.equals("watched")) type = 1;
            else if (param.equals("unwatched")) type = 2;
            else if (param.equals("all")) type = 3;
         ...
         * <state>
         *
         * <p>!watched <anime> <epnumber> !watched <fid> !watched <ed2k link> !watched <anime> upto
            ...
            try {
                ...
                if (j >= 0) {
                    ...
                    if (j < 0) {
                        ...
                        if (j < 0) {
                            j = s.indexOf(" upto ");
                            if (j > 0) epno = -Integer.parseInt(s.substring(j + 6).trim());
                            else {
                                j = s.lastIndexOf(' ');
                                if (j < 0) j = 0;
                                try {
                ...
                if (r.code == AConR.MYLIST_ENTRY_EDITED) {
                    if (r.data.length() > 3) return "WATCHED: entry updated.";
        ...
        public String state(String s) {
            try {
                ...
                if (j >= 0) {
                    ...
                    if (j < 0) {
                        j = s.indexOf(" upto ");
                        if (j > 0) epno = -Integer.parseInt(s.substring(j + 6, i).trim());
                        else {
                            j = s.lastIndexOf(' ', i - 1);
                            if (j < 0) j = 0;
                ...
                AConR r = m_ac.send("MYLISTADD", api, true);
                if (r.code == AConR.MYLIST_ENTRY_EDITED) {
                    if (r.data.length() > 3) return "STATE: entry updated.";
