...
public class AFile extends Base {
    ...
    public AFile(String s[]) {
        ...
        aid = U.i(s[i++]);
        eid = U.i(s[i++]);
        if (s[i].length() < 1) gid = 0;
        else gid = U.i(s[i]);
        i++;
        lid = U.i(s[i++]);
        stt = U.i(s[i++]);
        if (s[i].length() < 1) mLs = 0;
    ...
    private String url0(String str, boolean non) {
        if (non) return "http://" + A.S_WEB + "/perl-bin/animedb.pl?" + str + "&nonav=1";
    ...
    public String urlYear() {
        try {
            return url0(
                    "do.search=%20Start%20Search%20&show=search&noalias=1&search.anime.year="
                            + anime.yea,
                    false);
        } catch (Exception e) {
            return url1("search");
    ...
    public String getInvalid() {
        if ((stt & F_CRCERR) == F_CRCERR) return "invalid crc";
        return "";
    }
    ...
    public String mds() {
        ...
        if (md5 == null || md5.length() < 1 || sha == null || sha.length() < 1) x += 'h';
        if (len < 1) x += 'l';
