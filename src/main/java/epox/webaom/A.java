...
public class A {
    ...
    public static void init() {
        ...
        if (fschema == null) fschema = A.getFileString("file.htm");
    ...
    public static boolean shutdown(boolean opx) {
        if (opx) {
            ...
            if (o.onDisk()) {
                gui.opts(o);
                if (!A.opt.equals(o))
                    if (o.getB(Options.B_AUTOSAV)) o.save();
                    else
                        switch (yes_no_cancel(
                                "The options has changed", "Do you want to save them?")) {
                            case 0:
                                o.save();
                                break;
                            case -1:
