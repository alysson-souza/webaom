...
/**
 * Validates application startup conditions and provides graceful degradation
 * when optional features (logging, database) are unavailable.
 */
public class StartupValidator {
	 ...
	 *
	 * @param autoLogEnabled whether auto-logging is enabled
	 * @param logFilePath the configured log file path (can be null/empty for defaults)
	...
	/**
	 * Validate database configuration and provide helpful information.
	 * Only reports actual problems - H2 fallback is silent and expected behavior.
	 *
	 * @param autoLoadDBEnabled whether auto-loading DB is enabled
	 * @param dbUrl the configured database URL
	...
	public static List<StartupIssue> validateDatabase(boolean autoLoadDBEnabled, String dbUrl) {
		...
		if (!dbUrlLower.contains("postgresql") &&
		    !dbUrlLower.contains("mysql") &&
		    !dbUrlLower.contains("h2") &&
	 ...
	 * Returns a list of issues found during startup.
	 *
	 * @param options the Options instance to validate
	...
	public static List<StartupIssue> validateStartup(Options options) {
		...
		if (autoLog && validatedLogPath == null && logPath != null && !logPath.trim().isEmpty()) {
			issues.add(new StartupIssue(
				...
				"Automatic logging has been disabled. You can enable it again in Options when the issue is resolved."
			));
		}
	 ...
	 *
	 * @param issues list of startup issues
	...
	public static boolean hasErrors(List<StartupIssue> issues) {
		...
		for (StartupIssue issue : issues) {
			if (issue.getSeverity() == StartupIssue.Severity.ERROR) {
				return true;
			}
		}
		return false;
