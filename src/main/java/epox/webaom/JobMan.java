...
public class JobMan {
    public static void setPath(Job job, String path, boolean parent) {
        synchronized (job) {
            ...
            if (job.getHealth() == Job.H_MISSING) {
                ...
                if (job.m_fn != null) status = Job.MOVEWAIT;
    ...
    public static void setName(Job job, String s) {
        synchronized (job) {
            ...
            if (job.getHealth() < Job.H_DELETED
                ...
                if (job.m_fc.renameTo(f)) JobMan.setJobFile(job, f);
        ...
        synchronized (job) {
            ...
            switch (status) {
                case Job.HASHED:
                    ...
                    if (job.m_fa == null) j = Job.IDENTWAIT;
                    else j = Job.FINISHED;
                    if (job.mBf) {
                        if (A.db._ok()
                                && !A.db.update(0, job, DB.I_J)) { // add job to db, if it exist...
                            j = A.db.getJob(job, true); // then retrieve
                            if (j >= 0) { // if job exist in db... (should be true, always)
                                A.cache.gatherInfo(
                                        job, true); // retrieve info (like anime, group, etc.)
                    ...
                    break;
                case Job.IDENTIFIED:
                    if (job.m_fa == null || job.m_fa.fid == 0 || !updatePath(job)) j = Job.FAILED;
                    else if (A.autoadd && job.mIlid == 0) j = Job.ADDWAIT;
                    else if (job.m_fn != null) j = Job.MOVEWAIT;
                    else j = Job.FINISHED;
                    break;
                case Job.ADDED:
                    if (job.m_fa == null || job.m_fa.fid == 0) return;
                    if (job.m_fn != null) j = Job.MOVEWAIT;
                    else j = Job.FINISHED;
            ...
            if (!job.mBf && job.check(Job.F_DB)) A.db.update(0, job, DB.I_J);
        }
    }
    ...
    public static boolean updatePath(
            Job job, File f) { // only want to use renameTo if same partition
        job.m_fn = null;
        ...
        if (normal && f.exists()) {
            ...
            A.gui.println(
                    sf0 + " cannot be moved to " + sf1 + ": Destination file already exists!");
            return false;
        }
    ...
    private static void moveSub(File a, File b, String ext) {
        ...
        if (c.exists()) c.renameTo(new File(changeExt(b, ext)));
    }
