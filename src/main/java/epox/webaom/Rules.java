...
public class Rules {
    ...
    public File apply(Job j) {
        ...
        if (j.m_fa.anime != null) {
            ...
            if (smov != null) {
                String p = fin(j, smov);
                if (p.startsWith("." + File.separator)) path += p.substring(1);
                else path = p;
            }
            if (sren != null) name = fin(j, sren) + "." + j.getExtension();
        } else return null;
    ...
    private Vector /* !<Section> */ build(String script, Job j) throws Exception {
        ...
        while (st.hasMoreTokens()) {
            ...
            else if (tup.startsWith("ELSE IF ")
                    && !prev
                    && (prev = check(tok.substring(8, (i = tup.indexOf(" DO "))), j))
                    && handle(schema, tok.substring(i + 4))) break;
            else if (tup.startsWith("ELSE DO ") && !prev && handle(schema, tok.substring(8))) break;
        }
        return schema;
    }
    ...
    private boolean handle(Vector /* !<Section> */ sch, String op) throws Exception {
        ...
        if (up.equals("FINISH")) return true;
        ...
        if (up.startsWith("ASSUME ")) {
            try {
                if (up.startsWith("ASSUME SPECIAL ")) A.ASSP = U.i(up.substring(15).trim());
        ...
        if (up.startsWith(TRUNC)) {
            int x = up.indexOf('>');
            int y = up.indexOf(',');
            if (x > 0 && y > 0 && y < x) sch.add(new Section(up.substring(0, x + 1)));
            else A.gui.println(Hyper.error("Invalid rule element: " + op));
            return false;
        }
        sch.clear();
        throw new Exception(
                "Error after DO: Expected SET/ADD/FAIL/FINISH/TRUNCATE<int,str>: " + op);
    }
    ...
    private static String replace(String s, Vector v) {
        ...
        for (int i = 0; i < v.size(); i++) {
            ...
            if (ds.sel.booleanValue()) s = U.replace(s, ds.src, ds.dst);
        }
        return s;
    }
    ...
    private static String truncate(String s) {
        try {
            ...
            while (x > 0) {
                ...
                t = s.substring(z + 1, y);
                if (x > i + t.length()) s = s.substring(0, i - t.length()) + t + s.substring(y + 1);
...
class Section {
    ...
    public Section(String s) {
        ...
        if (i >= 0 && j > i) str = str.substring(i + 1, j);
        i = str.indexOf("//");
