...
public class TableSorter {
    ...
    public void sort(int[] array, int col, boolean refresh) {
        ...
        if (refresh) {
            sort0(array, 0, num - 1);
            m_desc = m_swit;
            if (m_desc) invert(array);
        } else {
            if (m_cmp0 != col) m_cmp1 = m_cmp0;
            m_cmp0 = col;

            if (col == m_last_col || m_last_n < 0) m_swit = !m_swit;
        ...
        if (type.getSuperclass() == Number.class)
            return (int)
                    (((Number) o1).floatValue()
                            - ((Number) o2).floatValue()); // compare((Number)o1, (Number)o2);
        else if (type == String.class) return ((String) o1).compareToIgnoreCase((String) o2);
